c    This is, basically, a copy of dqng from quadpack with the only quirk
c    that the integrand evaluations are vectorized. So that the function
c    f is not a function anymore, but a subroutine with the signature
c
c    subroutine evalf(n, x, f)
c
c    where n is an integer and x, f are arrays of the length n, containing
c    the abscissae in x (on entry) and the function values in f on exit.
c
c    These modifications are (c) 2023 Konstantin L. Metlov <metlov@donfti.ru>
c    and distributed under the same terms as the original code.
c

      subroutine vdqng(evalf,a,b,epsabs,epsrel,result,abserr,neval,ier)
c***begin prologue  vdqng
c***date written   800101   (yymmdd)
c***revision date  230426   (yymmdd)
...
c
c           evalf    - subroutine defining the integrand function.
c                    it must have the signature evalf(n, x, f). On entry
c                    n specifies the integer number of points to compute
c                    and x contains n abscissae of the integrand in the
c                    ascending order. The subrouting must compute n
c                    corresponding function values and store them in f.
c                    The actual name of this subroutine must be declared
...
c
     ...
     *  savfun(21)
c
      double precision fbuf,xbuf
      dimension fbuf(44),xbuf(44)
...
c
    ...
    5 res10 = 0.0d+00
      ...
      do 7 k=1,5
        absc = hlgth*x1(k)
        xbuf(k)=centr-absc
        xbuf(11-k)=centr+absc
   7  continue
      call evalf(10,xbuf,fbuf)
      do 10 k=1,5
        fval1 = fbuf(11-k)
        fval2 = fbuf(k)
        fval = fval1+fval2
...
c
   ...
   30 continue
      do 35 k=1,11
        absc = hlgth*x3(k)
        xbuf(k)=centr-absc
        xbuf(23-k)=centr+absc
   35 continue
      call evalf(22,xbuf,fbuf)
      do 40 k=1,11
        ipx = ipx+1
        fval = fbuf(23-k)+fbuf(k)
...
c
   ...
   50 continue
      do 55 k=1,22
        absc = hlgth*x4(k)
        xbuf(k)=centr-absc
        xbuf(45-k)=centr+absc
   55 continue
      call evalf(44,xbuf,fbuf)
      do 60 k=1,22
        res87 = res87+w87b(k)*(fbuf(45-k)+fbuf(k))
   60 continue
      result = res87*hlgth
...
c ***jump out of do-loop
   ...
   70 continue
   80 call xerror("abnormal return from dqng",ier,0)
  999 return
      end
